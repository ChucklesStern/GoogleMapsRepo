...
scopeApp.controller('scopeController', ['$scope', '$firebaseArray', '$firebaseObject', 'NgMap', '$timeout', function($scope, $firebaseArray, $firebaseObject, NgMap, $timeout, $uibModal) {
    ...
    vm.checker = function(stateOption) {
        var finishClean;
      console.log("checker hit!")
    console.log($scope.markerArray2);
    console.log($scope.markerArray2.length);
    console.log("map");
    console.log(vm.map);
if ($scope.markerArray2.length > 0) {
                console.log("GREATER THAN ZERO!!!!!!!!!!")
                console.log($scope.markerArray2);
                for (var i = 0; i > $scope.markerArray2.length; i++) {
                    $scope.markerArray2[i].setMap(null);
                    $scope.markerArray2[i].setVisible(false);
            ...
            console.log("marker Array after")
            console.log($scope.markerArray2);
            if (finishClean == true) {
        $timeout(vm.lookState(stateOption),500);    
   }
        }
 ...
 console.log('markers', vm.map.markers);
            ...
            for (var shop in shops) {
               ...
               var marker = new Marker({
                ...
                $scope.markerArray2.push(marker);
           

            }
            console.log("markerArray2.length");
            console.log($scope.markerArray2.length)
            vm.counter = $scope.markerArray2;

            var markerCluster = new MarkerClusterer(map, $scope.markerArray2);
